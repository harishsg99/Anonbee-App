...
class _ClubScreenState extends State<ClubScreen> {
  ...
  Widget toolbar(){
    if(widget.club.status == "finished"){
      ...
      if(widget.club.createdBy==phone){
        return Container(
          padding: EdgeInsets.symmetric(horizontal: 20,vertical: 12),
          height: 80,
          child: Row(
                ...
                ElevatedButton.icon(
                  ...
                  onPressed: ()async{
                   await FirebaseFirestore.instance.collection('clubs').doc(widget.club.clubId).update({
                    ...
                    await initialize();
              }, icon: Icon(CupertinoIcons.mic_circle_fill), label: Text("Start the Club"),)
            ],),
        );
      }else{
       ...
       return Container(
         ...
         child: Row(
           ...
           children: [
             ...
             widget.club.createdBy == phone ? ElevatedButton.icon(  style: ElevatedButton.styleFrom(
                    primary: Colors.red
                  ),onPressed: (){
                  FirebaseFirestore.instance.collection('clubs').doc(widget.club.clubId).update({
                    'status':'finished'
                  ...
                  Navigator.pop(context);
             }, icon: Icon(Icons.exit_to_app), label: Text("End Club")): ElevatedButton.icon(  style: ElevatedButton.styleFrom(
                    primary: Colors.amber
                  ),onPressed: (){
                  Navigator.pop(context);
             }, icon: Icon(Icons.exit_to_app), label: Text("Exit Club"))
